		...
		<div class="container">
			...
			<div class="nav-collapse">
				...
				$("#btn_logout").click(function(e) {
					...
					if(res){
						window.location.href="/admin.php/Home/Article/logout"
					}
           		});
			</script></li>
				</ul>
        ...
        if($(this).attr("name")==CONTROLLER_NAME){
			...
			<div class="span9">
				...
				<div class="well blank-slate" id="gbodx">
					<p>点击按钮创建文章</p>
					<a href="javascript:;" class="btn btn-primary" id="btn_addnew"><i	class="icon-plus icon-white"></i> 创建文章</a>
					<script>
				...
				<table id="tcontent" class="table table-bordered table-striped"style="text-align: left;">
					<thead>
						<tr>
							<th >ID</th>
							<th >文章标题</th>
					...
					<form class="form-horizontal">
						<fieldset>
							...
							<div class="control-group">
								...
								<div class="controls">
									<input type="text" class="input-xlarge" id="atitle"
										placeholder="请输入文章标题..." />
								</div>
							</div>
							<div class="control-group">
								...
								<div class="controls">
									<select id="fid" class="input-xlarge">
										<?php if(is_array($cate_list)): $i = 0; $__LIST__ = $cate_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cate): $mod = ($i % 2 );++$i;?><option value="<?php echo ($cate["id"]); ?>" disabled>---------<?php echo ($cate["catetitle"]); ?>---------</option>
										<?php if(is_array($cate['sub'])): $i = 0; $__LIST__ = $cate['sub'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ii): $mod = ($i % 2 );++$i;?><option value="<?php echo ($ii["id"]); ?>"><?php echo ($ii["catetitle"]); ?></option><?php endforeach; endif; else: echo "" ;endif; endforeach; endif; else: echo "" ;endif; ?>
									</select>
								</div>
							</div>
							<div class="control-group">
								...
								<div class="controls">
									<select id="zhuanti">
										...
										<?php if(is_array($cate_list)): $i = 0; $__LIST__ = $cate_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cate1): $mod = ($i % 2 );++$i;?><option value="<?php echo ($cate1["id"]); ?>" disabled>---------<?php echo ($cate1["catetitle"]); ?>---------</option>
										<?php if(is_array($cate1['sub'])): $i = 0; $__LIST__ = $cate1['sub'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ii1): $mod = ($i % 2 );++$i; if($ii1['zhuanti'] != 0): ?><option value="<?php echo ($ii1["id"]); ?>"><?php echo ($ii1["catetitle"]); ?></option>
										<?php else: ?>
											<option disabled="disabled" value="<?php echo ($ii1["id"]); ?>"><?php echo ($ii1["catetitle"]); ?></option><?php endif; endforeach; endif; else: echo "" ;endif; endforeach; endif; else: echo "" ;endif; ?>
									</select>
								</div>
							</div>
							<div class="control-group">
								...
								<div class="controls">
									<textarea class="input-xlarge" id="acont_top" rows="4"
							...
							<script>
								...
								$("#btn_save").click(function(e) {
									...
									if(id=='' || fid=='' || acont_top==''||acont==''||atitle==''){
										balert('warning','信息不完整');
										return;
									}
                                    var URL='/admin.php/Home/Article';
									$.ajax({
										type:'POST',
